...
class SSDfgInst : public SSDfgNode {
 public:
  ...
  SSDfgInst(SSDfg* ssdfg, dsa::OpCode inst, bool is_dummy = false)
    ...
    for (int i = _values.size(); i < dsa::num_values(_ssinst); ++i) {
  ...
  dsa::OpCode inst() { return _ssinst; }
  ...
  virtual int bitwidth() override { return dsa::bitwidth[_ssinst]; }
