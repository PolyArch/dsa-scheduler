...
class Capability {
 public:
  struct Entry {
    ...
    int encoding;
    bool count;
    Entry(OpCode op, int encoding, bool count)
        : op(op), encoding(encoding), count(count) {}
  };

  void Add(OpCode op, int encoding, bool count = true) {
    for (auto& elem : capability) {
      CHECK(elem.encoding != encoding)
          << elem.encoding << ", Op encoding already occupied!";
    }
    for (auto& elem : capability) {
      if (elem.op == op) {
        // std::cerr << "Warning: " << name_of_inst(op) << " already added to " << name <<
        // std::endl;
        return;
      }
    }
  ...
  void Erase(int j) { capability.erase(capability.begin() + j); }
