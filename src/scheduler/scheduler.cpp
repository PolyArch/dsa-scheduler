...
bool HeuristicScheduler::assignVectorInputs(SSDfg* ssPDG, Schedule* sched) {
  ...
  for (int j = 0; j < n; ++j) {
    SSDfgVecInput *vec_in = ssPDG->vec_in(j);
    ...
    for (auto &i : sd) {
      ...
      if (vec_in->inputs().size() <= vport_desc.size() && sched->vportOf(vport_id) == nullptr) {
        ...
        for (unsigned m = 0; m < vec_in->inputs().size(); ++m) {
          ...
          SSDfgNode *sspdg_input = vec_in->inputs()[m];
          sched->assign_node(sspdg_input, make_pair(0, cgra_in_port));
        }
...
bool HeuristicScheduler::assignVectorOutputs(SSDfg* ssPDG, Schedule* sched) {
  SS_CONFIG::SubModel *subModel = _ssModel->subModel();
  ...
  for (int j = 0; j < n; ++j) {
    SSDfgVecOutput *vec_out = ssPDG->vec_out(j);
    ...
    for (auto &i : sd) {
      ...
      if (vec_out->outputs().size() <= vport_desc.size() && sched->vportOf(vport_id) == nullptr) {
        ...
        for (unsigned m = 0; m < vec_out->outputs().size(); ++m) {
          ...
          //Get the input pdgnode corresponding to m
          SSDfgNode *sspdg_output = vec_out->outputs()[m];
          if (sched->pdgNodeOf(cgra_out_port) != nullptr) {
            ports_okay_to_use = false;
            break;
          }
          std::pair<int, int> curScore = scheduleHere(sched, sspdg_output, make_pair(0, cgra_out_port), candRouting);
          if (curScore >= fscore) { //?????
            ports_okay_to_use = false;
            break;
          }
        ...
        for (unsigned m = 0; m < vec_out->outputs().size(); ++m) {
          ...
          //Get the input pdgnode corresponding to m
          SSDfgNode *sspdg_output = vec_out->outputs()[m];
          sched->assign_node(sspdg_output, make_pair(0, cgra_out_port));
        }
        //Perform the vector assignment
...
void HeuristicScheduler::apply_routing(Schedule *sched, CandidateRouting *candRouting) {
  ...
  for (auto elem : candRouting->routing) {
    ...
    if (ssfu *fu = dynamic_cast<ssfu *>(dest)) {
      SSDfgNode *dest_pdgnode = sched->pdgNodeOf(fu);
      if (!dest_pdgnode) {
    ...
    //cout<<"pdgnode: "<< I->second->def()->name()<<" sslink: "<<link->name()<<endl; 
    for (auto &e : elem.second) {
...
void HeuristicScheduler::apply_routing(Schedule *sched, SSDfgNode *pdgnode,
                                       pair<int, ssnode *> here, CandidateRouting *candRouting){
  
  //cout << "pdgnode: " << pdgnode->name()  << " ssnode: " << here.second->name()
  ...
  if(SSDfgInst* inst = dynamic_cast<SSDfgInst*>(pdgnode)) {
  ...
  if (auto vec = dynamic_cast<SSDfgVec*>(pdgnode)) {
...
vector<pair<int, ssnode*>> HeuristicScheduler::fill_input_spots(Schedule* sched, SSDfgInput* pdginst) {
...
vector<pair<int, ssnode*>> HeuristicScheduler::fill_output_spots(Schedule* sched,SSDfgOutput* pdginst) {
  vector<pair<int, ssnode*>> spots;
...
vector<pair<int, ssnode*>> HeuristicScheduler::fill_inst_spots(Schedule *sched, SSDfgInst *pdginst) {
  vector<pair<int, ssnode*>> spots;
...
//void Scheduler::unroute(Schedule* sched, SSDfgEdge* pdgedge, 
...
int HeuristicScheduler::routing_cost(SSDfgEdge* edge, int from_slot, int next_slot, sslink* link,
  ...
  SSDfgNode *pdgnode = edge->def();
  ssnode *next = link->dest();

  //check if connection is closed..
  SSDfgNode *sched_node = sched->pdgNodeOf(next_slot, link);
...
pair<int,int>
HeuristicScheduler::route_minimize_distance(Schedule *sched, SSDfgEdge *pdgedge,
...
bool Scheduler::check_res(SSDfg* ssPDG, SSModel* ssmodel) {
  int dedicated_insts = 0;
  int temporal_insts = 0;
  for (auto i : ssPDG->inst_vec()) {
    if (!i->is_temporal()) {
      dedicated_insts++;
    } else {
      temporal_insts++;
  ...
  for (auto &pair : dedicated_count_types) {
    ...
    if (fu_count < pdg_count) {
      ...
      cerr << "Error: PDG has " << pdg_count << " " << name_of_inst(ss_inst)
