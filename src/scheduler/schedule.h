...
public:
  ...
  //Read in schedule (both ssmodel, sspdg, and schedule from file)
  Schedule(SSModel *model, SSDfg *pdg) : _ssModel(model), _ssPDG(pdg) {
    allocate_space();
  }

  Schedule(SSModel *model) : _ssModel(model), _ssPDG(nullptr) {
    allocate_space();
  }
  ...
  //Rest of Stuff
  SSDfg *sspdg() const { return _ssPDG; }
  ...
  void assign_lat(SSDfgNode *pdgnode, int lat) {
    _vertexProp[pdgnode->id()].lat = lat;
  }

  int latOf(SSDfgNode *pdgnode) {
  ...
  void assign_lat_bounds(SSDfgNode *pdgnode, int min, int max) {
    auto &vertex_prop = _vertexProp[pdgnode->id()];
    vertex_prop.min_lat = min;
    vertex_prop.max_lat = max;
  }

  std::pair<int, int> lat_bounds(SSDfgNode *pdgnode) {
    auto &vertex_prop = _vertexProp[pdgnode->id()];
  ...
  //Assign the ssnode to pdgnode and vice verse
  void assign_node(SSDfgNode *pdgnode, std::pair<int, ssnode *> assigned) {
    int vid = pdgnode->id();
    ...
    assert(pdgnode->type() < SSDfgNode::V_NUM_TYPES);
    _num_mapped[pdgnode->type()]++;
    assert(_num_mapped[SSDfgNode::V_INPUT] <= _ssPDG->inputs().size());
    assert(_num_mapped[SSDfgNode::V_OUTPUT] <= _ssPDG->outputs().size());
    assert(_num_mapped[SSDfgNode::V_INST] <= _ssPDG->inst_vec().size());
  ...
  void calc_out_lat() {
    for (int i = 0; i < _ssPDG->num_vec_output(); ++i) {
      calc_out_vport_lat(_ssPDG->vec_out(i));
    }
  }

  void calc_out_vport_lat(SSDfgVecOutput *pdgvec_out) {
    int max_lat = 0;
    for (auto pdgout : pdgvec_out->outputs()) {
      //ssnode* out_ssnode = locationOf(pdgout).first;
      int lat_of_out_ssnode = _vertexProp[pdgout->id()].lat;
      std::cout << pdgvec_out->gamsName() << " lat:" << lat_of_out_ssnode << "\n";
  ...
  std::vector<bool> maskOf(SSDfgVec *pdgvec) {
  ...
  //vector to port num
  //true for input
  void assign_vport(SSDfgVec *pdgvec, std::pair<bool, int> pn,
  ...
  void unassign_input_vec(SSDfgVecInput *pdgvec) {
  ...
  //unassign all the input nodes and vector
  void unassign_output_vec(SSDfgVecOutput *pdgvec) {
  ...
  void unassign_pdgnode(SSDfgNode *pdgnode) {
  ...
  //pdf edge to sslink
  void assign_edgelink(SSDfgEdge *pdgedge, int slot, sslink * slink) {
    assert(slink);
    assert(pdgedge);
  ...
  //void print_links(SSDfgEdge* pdgedge) {
  ...
  int link_count(SSDfgEdge *pdgedge) {
  ...
  //find first node for
  SSDfgNode *pdgNodeOf(int slot, sslink* link) {
    assert(link);
  ...
  //find first node for
  SSDfgNode *pdgNodeOf(ssnode *node) {
  ...
  std::unordered_set<std::pair<int, SSDfgNode *>,
          boost::hash<std::pair<int, SSDfgNode *>>> pdg_nodes_of(ssnode *node) {
    auto &vec = _nodeProp[node->id()].vertices;
    return vec;
  }

  ssnode *locationOf(SSDfgNode *pdgnode) {
    return _vertexProp[pdgnode->id()].node;
  }

  std::pair<int, ssnode *> location_of(SSDfgNode *pdgnode) {
    return std::make_pair(_vertexProp[pdgnode->id()].idx, _vertexProp[pdgnode->id()].node);
  }

  bool is_scheduled(SSDfgNode *pdgnode) {
  ...
  bool fixDelay(SSDfgOutput *pdgout, int ed, std::unordered_set<SSDfgNode *> &visited);

  void checkOutputMatch(int &latmis);

  void calcAssignEdgeLink_single(SSDfgNode *pdgnode);
        ...
        for (auto &inlink: sssw->in_links()) {
          for (int slot_in = 0; slot_in < 8; ++slot_in) {
            if (linkAssigned(slot_in, inlink)) {
              ...
              SSDfgNode *innode = pdgNodeOf(slot_in, inlink);
  ...
  void reset_lat_bounds() {
    for (auto elem : _ssPDG->inputs()) {
      auto &vp = _vertexProp[elem->id()];
      vp.min_lat = 0;
      vp.max_lat = 0;
    }
    for (auto elem : _ssPDG->inst_vec()) {
      auto &vp = _vertexProp[elem->id()];
      vp.min_lat = 0;
...
private:
  ...
  void reconstructSchedule(
          ...
          std::map<ssnode *, SSDfgNode *> &pdgnode_for,
...
public:
  ...
  int inputs_complete() { return num_inputs_mapped() == _ssPDG->inputs().size(); }

  int outputs_complete() { return num_outputs_mapped() == _ssPDG->outputs().size(); }

  int insts_complete() { return num_insts_mapped() == _ssPDG->inst_vec().size(); }

  int isComplete() { return num_mapped() == _ssPDG->nodes().size(); }
  ...
  int num_left() {
    int num = _ssPDG->nodes().size() - num_mapped();
    assert(num >= 0);
    return num;
  }
  ...
  void allocate_space() {
    if (_ssPDG) {
      _vertexProp.resize((size_t) _ssPDG->num_node_ids());
      _edgeProp.resize((size_t) _ssPDG->num_edge_ids());
    }
    if (_ssModel) {
