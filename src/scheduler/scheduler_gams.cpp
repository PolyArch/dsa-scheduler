...
void GamsScheduler::print_mipstart(ofstream& ofs, Schedule* sched, SSDfg* ssDFG,
  ...
  for (auto dfgnode : ssDFG->nodes<SSDfgNode*>()) {
    ...
    ofs << "Mn.l('" << dfgnode->gamsName() << "','" << spot->gams_name(config)
        << "')=1;\n";
    ...
    for (auto dfgedge : dfgnode->uses()) {
      ...
      for (auto elem : link_set) {
        ...
        ofs << "Mvl.l('" << dfgnode->gamsName() << "','" << link->gams_name(config)
            << "')=1;\n";
        ofs << "Mel.l('" << dfgedge->gamsName() << "','" << link->gams_name(config)
            << "')=1;\n";
  ...
  // ofs << "Tv.l(v2) = smax((v1,e)$(Gve(v1,e) and Gev(e,v2)),Tv.l(v1) + sum(l,Ml.l(e,l)))
  // + delta.l(e);\n";
  //
  ofs << "Nl.l(l) = 1 - sum(v,Mvl.l(v,l));\n";
  ofs << "length.l=smax(v,Tv.l(v));\n";
  ofs << "cost.l = length.l;\n";
  //  ofs << "cost.l = 1000000* sum((iv,k)$kindV(K,iv),(1-sum(n$(kindN(K,n)), Mn.l(iv,
  //  n)))) +  1000
  //  * length.l + sum(l,sum(v,Mvl.l(v,l)));\n";
  ofs << "display Tv.l;\n";
...
bool GamsScheduler::schedule_internal(SSDfg* ssDFG, Schedule*& schedule) {
  ...
  string hw_model = string((char*)gams_models_hw_model_gms, gams_models_hw_model_gms_len);
  string timing_model =
      string((char*)gams_models_timing_model_gms, gams_models_timing_model_gms_len);
  string stage_model =
      string((char*)gams_models_stage_model_gms, gams_models_stage_model_gms_len);
  string softbrain_gams =
      string((char*)gams_models_softbrain_gams_gms, gams_models_softbrain_gams_gms_len);
  ...
  if (!_gams_files_setup) {
    ...
    ofs_constraints
        << "set stages_opt /place_heur_deform, place_heur_pos, fixR, fixM, MRT, MR, "
           "mipstart, passthrough, sll/;\n";
  ...
  _ssModel->subModel()->PrintGamsModel(ofs_ss_model, gamsToNode, gamsToLink, gamsToSwitch,
                                       gamsToPortN, 1 /*nconfigs*/);
  ...
  string line, edge_name, vertex_name, switch_name, link_name, out_link_name, ssnode_name,
      list_of_links, latency_str;
  ...
  while (gamsout.good()) {
    ...
    if (parse_stage == PortMap) {
      ...
      unsigned size_of_vp;
      size_of_vp =
          _ssModel->subModel()->io_interf().vports_map[pn.first][pn.second]->size();
  ...
  if (!message_start) {
    ...
    cerr << "\n\nError: Combination of FUs requested are NOT satisfiable with given "
            "SSCONFIG.\n\n";
    exit(1);
