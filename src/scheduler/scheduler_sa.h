...
public:
  ...
  std::pair<int, int> route(Schedule *sched, SSDfgEdge *pdgnode,
  ...
  bool schedule_internal(SSDfg *ssPDG, Schedule *&sched);

protected:
  std::pair<int, int> obj(Schedule*& sched, int& lat, 
      int& lat_mis, int& ovr, int& agg_ovr, int& max_util); 

  bool schedule_input(SSDfgVecInput *vec, SSDfg *ssPDG, Schedule *sched);

  bool schedule_output(SSDfgVecOutput *vec, SSDfg *ssPDG, Schedule *sched);

  bool scheduleNode(Schedule *sched, SSDfgNode *pdgnode);
  ...
  bool map_to_completion(SSDfg *ssPDG, Schedule *sched);

  bool map_io_to_completion(SSDfg *ssPDG, Schedule *sched);

  bool map_one_input(SSDfg *ssPDG, Schedule *sched);

  bool map_one_inst(SSDfg *ssPDG, Schedule *sched);

  bool map_one_output(SSDfg *ssPDG, Schedule *sched);

  void unmap_one_input(SSDfg *ssPDG, Schedule *sched);

  void unmap_one_inst(SSDfg *ssPDG, Schedule *sched);

  void unmap_one_output(SSDfg *ssPDG, Schedule *sched);

  void unmap_some(SSDfg *ssPDG, Schedule *sched);
