...
template<typename T> int calc_ovr(const std::unordered_set<T, boost::hash<T>> &vlist) {
  int total_used = 0;
  ...
  for (auto elem : vlist) {
    ...
    if (v->is_temporal()) {
      if (auto input = dynamic_cast<SbPDG_Input *>(v))
        in_vecs.insert(input->input_vec());
      if (auto output = dynamic_cast<SbPDG_Output *>(v))
        out_vecs.insert(output->output_vec());
    } else {
      total_used++;
    }
  }
