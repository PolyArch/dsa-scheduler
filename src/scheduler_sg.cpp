  ...
  for(int config = 0; config < sched->nConfigs(); ++config) {
    ...
    if ( r1 % 2 == 1) {
      ...
      while (!succ && attempt < spots.size()) {
        ...
        bestScore = scheduleHere(sched, pdgnode, cand_spot, config,*bestRouting,bestScore);
...
std::pair<int,int> SchedulerStochasticGreedy::scheduleHere(Schedule* sched, 
    ...
    int config, CandidateRouting& candRouting, 
...
void SchedulerStochasticGreedy::genRandomIndexBW(pair<bool, int>& vport_id, 
         vector<pair<int, vector<int>>>& vport_desc,  vector<pair<int,int>>& sd, 
         sbio_interface& si, unsigned int size, unsigned int index, 
         Schedule*& sched, StatType s) {
  unsigned int k;
  pair<int, int> p;
  int vport_num;
  unsigned int rep = 0;
