...
double FIFOEst(Metric metric, int depth) { return FIFO_COEF[(int)metric] * depth; }
...
struct Estimator : Visitor {
  Estimator(SSModel* arch_) : arch(arch_) {
    ...
    res(Metric::Area, Breakdown::Memory) +=
        iter->second.first + (arch->indirect() == 2) * 88800 + 5200;
    res(Metric::Power, Breakdown::Memory) +=
        iter->second.second + (arch->indirect() == 2) * 18.1 + 9.3;
  }
...
Result::Result() { memset(result, 0, sizeof result); }

double& Result::operator()(Metric x, Breakdown y) { return result[(int)x][(int)y]; }
