...
std::set<OpCode> Capability::find_all_insts_covered(OpCode source_inst) {
  ...
  std::string inst_name = dsa::name_of_inst(source_inst);
  ...
  for (std::string cand_inst_name : cover_possible_insts) {
    OpCode curr_ssinst_t = dsa::inst_from_string(cand_inst_name.c_str());
    if (curr_ssinst_t != dsa::SS_ERR) {
...
void Capability::determine_inst_accountability() {
      ...
      if (account) {
        ...
        if (!other_inst.count) {
          ...
          if (all_insts_covered_by_this.count(other_inst.op) > 0) {
            // std::cout << dsa::name_of_inst(other_inst_acc.first)
            //          << " covered by " << dsa::name_of_inst(inst) << "\n";
  ...
  //  std::cout << dsa::name_of_inst(inst_acc.first) << ": " << (inst_acc.second ?
  //  "used ":"not ") << " | ";
  //}
  // std::cout << "\n";
...
double Capability::get_area_from_inst_set() {
  ...
  for (auto elem : capability) {
    ...
    double curr_area = dsa::inst_area(elem.op);
    //std::cout << this->name << " " << name_of_inst(elem.op) << " " << curr_area << std::endl;
    if (curr_area < 0.0) {
      // std::cout << dsa::name_of_inst(inst) << " is not support for area
      // estimation"
      //          << std::endl;
...
double Capability::get_power_from_inst_set() {
  ...
  for (auto &elem : capability) {
    ...
    double curr_power = dsa::inst_power(elem.op);
    if (curr_power < 0.0) {
      // std::cout << dsa::name_of_inst(inst) << " is not support for power
      // estimation"
      //          << std::endl;
...
double Capability::power() {
  ...
  if (fu_name.find('-') != std::string::npos) {
    ...
    std::set<dsa::fu_type_t> primitive_fu_types;
    ...
    for (auto primitive_fu_name : primitive_fu_names) {
      fu_type_t primitive_fu_type =
          dsa::fu_type_from_string(primitive_fu_name.c_str());
      if (primitive_fu_type == dsa::fu_type_t::NON_PREDEFINED_FU_TYPE) {
    ...
    if (all_predefined) {
      ...
      for (fu_type_t predefined_fu_type : primitive_fu_types) {
        _power += _power + dsa::fu_type_power(predefined_fu_type);
      }
    } else {
  ...
  if (fu_type == dsa::fu_type_t::NON_PREDEFINED_FU_TYPE) {
    _power = get_power_from_inst_set();
  } else {
    _power = dsa::fu_type_power(fu_type);
  }
...
double Capability::area() {
  ...
  if (fu_name.find('-') != std::string::npos) {
    ...
    if (all_predefined) {
      ...
      for (fu_type_t predefined_fu_type : primitive_fu_types) {
        _area += _area + dsa::fu_type_area(predefined_fu_type);
      }
    } else {
  ...
  if (fu_type == dsa::fu_type_t::NON_PREDEFINED_FU_TYPE) {
    ...
    _area = dsa::fu_type_area(fu_type);
  }
