    ...
    // Parse all DSAGen Nodes
    {
      ...
      for(Json::Value::iterator dsaNode = jsonNodes.begin(); dsaNode!=jsonNodes.end(); ++dsaNode){
        ...
        if(nodeType == ADGKEY_NAMES[PE_TYPE]){
          ...
          DSA_CHECK(nodeTypeParam == nodeType)<< "Internal Node Type is " << nodeTypeParam 
          ...
          DSA_CHECK(compBits > 0 && compUnitBits > 0) << "Compute Bits = " << compBits 
          ...
          DSA_CHECK(resetRegParam.isArray()) << "FU" << localNodeId << ", reg size = " 
          << regSize
          ...
          // Get the defined max instruction latency (this is the manual latency, not the natural one)
          ...
          ssfu* fu = new ssfu(compBits, compUnitBits, fuInstSlotSize, fuOpDynamic, fuDelayFifoDepth, fu_type);
          ...
          // TODO: the fine grain subnet routing connectivity matrix is not used in scheduler
          // let me add it after the submission
          node = new ssswitch(compBits, compUnitBits,
          1/*Switch cannot be shared now*/, 
          ...
          // Node Type in Node Definition is different from the one in Node Name
          DSA_CHECK(nodeTypeParam == nodeType)
          << "NodeType from Name = " << nodeType <<", node type in parameter = " << nodeTypeParam;
          int nodeIdParam = nodeParam[ADGKEY_NAMES[NODEID]].asInt();
          // Node ID from node parameter should be same as the one from Node Name
          DSA_CHECK(nodeIdParam == localNodeId);
          // Get the compute bits
          int compBits = 64; // defined by connected compute node, for now I just assume 64
          // Get the compute unit bits
          ...
          node = vp;
          sf -> add_node(node);
          DSA_CHECK(globalNodeId == node->id()) << "Global ID = " << globalNodeId << ", node->id() = " << node->id();
          globalNodeId++;
    ...
    // Parse all DSAGen Links
    {
      ...
      for (int i = 0; i < jsonLinks.size(); ++i) {
        ...
        if(sourceNodeType.compare(ADGKEY_NAMES[PE_TYPE]) == 0){
          sourceModule = fu_tab[sourceNodeId]; sourceDefined = true;
        }else if(sourceNodeType.compare(ADGKEY_NAMES[SW_TYPE]) == 0){
          sourceModule = sw_tab[sourceNodeId];sourceDefined = true;
        }else if(sourceNodeType.compare(ADGKEY_NAMES[IVP_TYPE]) == 0){
          sourceModule = ivp_tab[sourceNodeId];sourceDefined = true;
        }
        ...
        if(sinkNodeType.compare(ADGKEY_NAMES[PE_TYPE]) == 0){
          sinkModule = fu_tab[sinkNodeId]; sinkDefined = true;
        }else if(sinkNodeType.compare(ADGKEY_NAMES[SW_TYPE]) == 0){
          sinkModule = sw_tab[sinkNodeId];sinkDefined = true;
        }else if(sinkNodeType.compare(ADGKEY_NAMES[OVP_TYPE]) == 0){
          sinkModule = ovp_tab[sinkNodeId];sinkDefined = true;
        }
        
        // Connect between node
        // TODO: since memory node is not included, so we only connect between IVP -> PE <-> SW -> OVP
        // I don't know why sourceModule && sinkModule not work, should it be nullptr if it is memory node?
        if(sourceDefined && sinkDefined){
          DSA_INFO << "Connect: " << sourceNodeType << "." << sourceNodeId <<"["<< sourceIndex << "]"
